    ...
    <div className="p-4 space-y-4">
      ...
      {result.errors?.length > 0 ? (
        <div className="space-y-3">
          ...
          <ul className="space-y-2 bg-gray-800/50 p-3 rounded-lg border border-green-500/30">
            {result.errors.map((err, index) => {
              ...
              if (err.message.length > 0) {
                return (
                  <li key={index} className="flex items-start gap-2">
                    ...
                    <div>
                      {err.line && err.column && (
                        <span className="font-mono text-xs text-gray-400">Line {err.line}, Col {err.column}</span>
                      )}
              ...
              return null; // Return null if message is empty or doesn't meet condition
            })}
          </ul>
        ...
        <div className="flex items-center gap-2 p-2 bg-green-900/30 rounded-lg border border-green-500/30">
          ...
          <span className="text-sm text-green-300">No syntax errors found</span>
        </div>
      )}
